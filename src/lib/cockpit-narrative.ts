...
import { KLARNA_FEW_SHOT } from '@/lib/cockpit-few-shot';
import type {
  CockpitData,
  CockpitReport,
  CockpitScores,
...
function buildFullPrompt(data: CockpitData, scores: CockpitScores): string {
  ...
  return `${KLARNA_FEW_SHOT}

You are a senior sell-side equity analyst producing a structured Investment Cockpit for ${company} (${ticker}${exchange ? ` on ${exchange}` : ''}).
...
export async function synthesizeCockpit(data: CockpitData): Promise<CockpitReport> {
  ...
  const SYNTHESIS_MODELS = ['claude-opus-4-7', 'claude-sonnet-4-6'];
  const SYSTEM_PROMPT =
  ...
  for (const model of SYNTHESIS_MODELS) {
    try {
      ...
      console.log(
        `[narrative] Claude (${model}) stop_reason:`,
      ...
      if (content?.type === 'text') {
        ...
        if (rawJson === null) {
          ...
          const raw = fenceMatch ? fenceMatch[1]! : content.text;
          try {
            JSON.parse(raw.trim());
          } catch (parseErr) {
            console.error(`[narrative] JSON.parse error (${model}):`, String(parseErr));
            console.error('[narrative] stop_reason was:', response.stop_reason);
            // JSON parse failed — fall through to next model if available
            if (model !== SYNTHESIS_MODELS[SYNTHESIS_MODELS.length - 1]) {
              console.log(`[narrative] Trying next model after parse failure`);
              continue;
            }
          }
        }
      }
      break; // success (or last model) — stop the loop
    } catch (error: unknown) {
      const errStr = String(error);
      const isOverloaded =
        errStr.includes('overloaded') || errStr.includes('529') || errStr.includes('529');
      console.error(`[narrative] Claude API call failed (${model})`, { error: errStr });
      if (!isOverloaded || model === SYNTHESIS_MODELS[SYNTHESIS_MODELS.length - 1]) {
        // Non-retryable error or last model — give up
        break;
      }
      console.log(`[narrative] Model ${model} overloaded — trying fallback model`);
